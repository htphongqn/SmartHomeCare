<%@ Page Title="" Language="C#" MasterPageFile="~/siteadmin.master" AutoEventWireup="true" CodeFile="admintrash.aspx.cs" Inherits="admintrash" %>
        ...
        function ConfirmEmpty() {
            ...
            jConfirm('<%= DBClass.GetMessageByCode2("empty_trash_comfirm") %>', 'Question?', function (r) {
                if (r) {
                    $.ajax({
                        type: "POST",
                        url: 'admintrash.aspx/EmptyTrashService',
                        data: JSON.stringify({ args: [{
                            Ids: Ids
                        }]
                        }),
            ...
            if (Ids == "") {
                ...
                jConfirm('<%= DBClass.GetMessageByCode2("delete_message") %>', 'Question?', function (r) {
                    if (r) {
                        $.ajax({
                            type: "POST",
                            url: 'admintrash.aspx/DeleteMessage',
                            data: JSON.stringify({ args: [{
                                Ids: Ids
                            }]
                            }),
...
<div class="gridview">
    <asp:GridView ID="grdTrash" runat="server" AllowPaging="True" 
            ...
            EmptyDataText="No message" DataKeyNames="AutoId" PageSize="15">
            <Columns>
                <asp:TemplateField>
                    <HeaderTemplate>
   ...
   <%-- <asp:Button Text="Delete" runat="server" ID="btnDelete" 
         ...
         <input id="Button2" type="button" value="Delete" onclick="ConfirmDelete()" />
    <div class="popupConfirmation" style="display:none;">
       ...
       <%-- <asp:Button ID="btnEmpty" runat="server" OnClientClick="return Confirm();" onclick="btnEmpty_Click" 
        Text="Empty" Visible="False" />
        <asp:Button ID="Button1" runat="server" onclientclick="return Confirm();" 
        Text="Button" Visible="False" />--%>
        <input id="btnEmpty" type="button" value="Empty" onclick="ConfirmEmpty()" />
